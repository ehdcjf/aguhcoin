...
export default function QuillEditor({ body, handleQuillChange }) {
  ...
  const onClickImageBtn = () => {
    ...
    input.onchange = async () => {
      ...
      const location = `http://localhost:3002/${imageURI}`;

      const range = quillInstance.current.getSelection(true);
      quillInstance.current.insertEmbed(range.index, "image", `${location}`);
  ...
  useEffect(() => {
    ...
    if (quillElement.current && window.Quill) {
      ...
      /* quill에서 text-change 이벤트 발생시에 setBody(innerHTML)을 통해 body를 업데이트합니다.
               body가 업데이트되어도 useEffect 발생 조건 인자([isError])에 body가 없으므로
